...
//! Define _IRR_COMPILE_WITH_SDL_DEVICE_ to compile with the SDL device.
//#define _IRR_COMPILE_WITH_SDL_DEVICE_
#ifdef NO_IRR_COMPILE_WITH_SDL_DEVICE_
#undef _IRR_COMPILE_WITH_SDL_DEVICE_
#endif

//! Define _IRR_COMPILE_WITH_CONSOLE_DEVICE_ to compile with the fallback console device.
...
#endif // !WINDOWS && !OSX && !IOS && !ANDROID && !EMSCRIPTEN
...
//! Define _IRR_MATERIAL_MAX_TEXTURES_ INT to set maximum number of texture an SMaterial can have, up to 8 are supported by Irrlicht.
#define _IRR_MATERIAL_MAX_TEXTURES_ 8

//! Define _IRR_COMPILE_WITH_XML_ whether to support XML and XML-based formats (irrmesh, collada...)
#define _IRR_COMPILE_WITH_XML_
#ifdef NO_IRR_COMPILE_WITH_XML_
#undef _IRR_COMPILE_WITH_XML_
#endif

//! Define _IRR_COMPILE_WITH_LEAK_HUNTER_ to add a leak-hunter to Irrlicht which helps finding unreleased reference counted objects.
...
//! Define _IRR_COMPILE_WITH_PROFILING_ to enable profiling information in the engine
...
// It's a subset of OGL ES2, so always needed when using WebGL
...
//! Define OPTIN_IRR_USE_NVIDIA_PERFHUD_ to opt-in to using the nVidia PerHUD tool
...
#ifndef OPTIN_IRR_USE_NVIDIA_PERFHUD_
#undef _IRR_USE_NVIDIA_PERFHUD_
#endif

//! Define  BURNINGVIDEO_RENDERER_* to one of the three setting for Burning's Video Software Rasterizer
...
//! Define IGNORE_DEPRECATED_WARNING if you want to ignore the deprecated warnings
...
//! Define IRRLICHT_FAST_MATH to set FPU settings
...
//! Define _IRR_STATIC_LIB_ to build Irrlicht as a static library.
//#define _IRR_STATIC_LIB_

#ifdef _IRR_WINDOWS_API_

	//! Define _IRR_HAS_PRAGMA_LIB if you want to disable pragma lib calls.
	#define _IRR_HAS_PRAGMA_LIB
	#ifdef NO_IRR_HAS_PRAGMA_LIB
	#undef _IRR_HAS_PRAGMA_LIB
	#endif
